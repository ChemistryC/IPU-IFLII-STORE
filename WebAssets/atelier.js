...
export async function createAtelier(ctx) {
  ...
  const dock=document.createElement('nav');dock.className='atelier-ui atelier-dock';dock.setAttribute('aria-label','Temple destinations');
  dock.innerHTML='<span>THE TEMPLE</span><button data-dest="hall">01 · Main hall</button><button data-dest="atelier">02 · New collection</button><button data-dest="reading">03 · Reading room</button><button data-dest="wardrobe">04 · Open wardrobe</button><button data-dest="cinema">05 · Cinema</button><button data-issue="0">Read Timeless ↗</button><button data-issue="1">Read Collection ↗</button><button data-issue="2">Read Archive ↗</button><button data-beat>Music uplights · on</button>';
  document.body.append(dock);dock.querySelector('[data-beat]').textContent='Music uplights · '+(beatController.stats().enabled?'on':'off');
  ...
  dock.addEventListener('click',e=>{e.stopPropagation();const b=e.target.closest('button');if(!b)return;if(b.hasAttribute('data-beat')){b.textContent='Music uplights · '+(beatController.toggle()?'on':'off');return;}if(b.dataset.dest)go(b.dataset.dest);if(b.dataset.issue!==undefined)openBook(+b.dataset.issue);});
  ...
  for(const el of [dock,modal])for(const name of ['mousedown','mouseup','touchstart','touchend','pointerdown'])el.addEventListener(name,e=>e.stopPropagation());
